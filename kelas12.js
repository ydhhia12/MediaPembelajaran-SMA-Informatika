...
document.addEventListener("DOMContentLoaded", () => {
  ...
  firebase.database().ref("materiTambahan/12").on("value", snapshot => {
    daftarMateriDiv.innerHTML = "";

    if (!snapshot.exists()) {
      daftarMateriDiv.innerHTML = "<p>Belum ada materi tambahan untuk kelas 12.</p>";
      return;
    }
  ...
  materiTambahanLS.filter(m => m.kelas === "12").forEach(m => {
